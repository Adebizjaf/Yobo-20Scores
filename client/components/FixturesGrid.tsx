...
const featuredFixtures: Array<{ id: string; sport: string; home: string; away: string; date: string; time: string; league: string; homeLogo?: string; awayLogo?: string }> = [
...
const sportLabels = { soccer: "Soccer", basketball: "Basketball", "american-football": "American football", baseball: "Baseball", hockey: "Hockey" } as const;
...
export default function FixturesGrid() {
  const { data: liveData } = useLiveScores("all", "all");
  ...
  const fixtures = liveData?.matches.length ? liveData.matches.slice(0, 5).map((match) => ({ id: match.id, sport: sportLabels[match.sport as keyof typeof sportLabels] ?? match.sport, home: match.home.name, away: match.away?.name ?? "TBD", date: formatFixtureDate(match.startTime), time: formatFixtureTime(match.startTime), league: match.league, homeLogo: match.home.logo, awayLogo: match.away?.logo })) : featuredFixtures;
  return (
      ...
      {fixtures.map((fixture) => (
        <div key={fixture.id} className="rounded-xl border bg-background p-4 shadow-sm">
          ...
          <div className="mt-3 grid min-w-0 grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] items-center gap-1">
            <Team name={fixture.home} logo={fixture.homeLogo ?? (fixture.sport === "Soccer" ? logos.get(fixture.home.toLowerCase()) : undefined)} />
            <span className="px-1 text-sm font-semibold text-foreground/70">vs</span>
            <Team name={fixture.away} logo={fixture.awayLogo ?? (fixture.sport === "Soccer" ? logos.get(fixture.away.toLowerCase()) : undefined)} align="right" />
          </div>
