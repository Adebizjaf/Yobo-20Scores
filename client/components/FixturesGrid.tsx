...
export default function FixturesGrid() {
  return (
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      {fixtures.map((f) => (
        <div key={f.id} className="rounded-xl border bg-background p-4 shadow-sm">
          <div className="flex items-center justify-between text-xs text-foreground/60">
            <span className="font-semibold uppercase tracking-wide">{f.league}</span>
            <span>
              {f.date} • {f.time}
            </span>
          </div>
          ...
          <a href="/premium" className="mt-4 inline-flex w-full items-center justify-center rounded-md bg-gradient-to-r from-blue-600 to-green-600 px-3 py-2 text-sm font-semibold text-white hover:opacity-90">
            Watch on Premium
          </a>
        </div>
      ))}
