...
import { type Sport } from "@shared/live-scores";
...
export default function LiveScoresTicker() {
  const { data, isLoading, error } = useLiveScores("all", "all");
  const items = useMemo(() => [...(data?.matches ?? [])].sort((a, b) => Number(b.status === "live") - Number(a.status === "live")).slice(0, 8), [data]);

  if (isLoading) return <div className="h-10 animate-pulse rounded-full bg-muted" aria-label="Loading live scores" />;
  if (error || items.length === 0) return <div className="flex min-h-10 items-center justify-between gap-3 text-sm text-muted-foreground"><span>{error ? "Live scores are temporarily unavailable." : "No current sports data is available."}</span><Link to="/live" className="shrink-0 font-semibold text-blue-600 hover:underline">View all scores</Link></div>;

  const cards = items.map((match) => {
    const isLive = match.status === "live";
    return <div key={match.id} className="flex shrink-0 items-center gap-3 rounded-full border bg-background px-4 py-2 shadow-sm">
      <span className="text-lg" aria-hidden="true">{sportEmojis[match.sport]}</span><span className="text-xs font-semibold uppercase text-foreground/70">{match.league}</span>
      ...
      <span className={isLive ? "inline-flex items-center gap-1 rounded bg-red-500/10 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-red-600" : "rounded bg-muted px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-muted-foreground"}>{isLive && <span className="h-1.5 w-1.5 animate-pulse rounded-full bg-red-600" />}{isLive ? match.clock ? `Live ${match.clock}` : "Live" : match.statusLabel}</span>
    </div>;
  });
