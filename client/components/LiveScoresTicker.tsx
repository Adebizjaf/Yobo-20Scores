...
export default function LiveScoresTicker() {
  const { data, isLoading, error } = useLiveScores("all", "live");
  ...
  if (error || items.length === 0) return <div className="flex min-h-10 items-center justify-between gap-3 text-sm text-muted-foreground"><span>{error ? "Live scores are temporarily unavailable." : "No matches are live right now."}</span><Link to="/live" className="shrink-0 font-semibold text-blue-600 hover:underline">View all scores</Link></div>;
  ...
  const cards = items.map((match) => <div key={match.id} className="flex shrink-0 items-center gap-3 rounded-full border bg-background px-4 py-2 shadow-sm">
    ...
    <span className="inline-flex items-center gap-1 rounded bg-red-500/10 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-red-600"><span className="h-1.5 w-1.5 animate-pulse rounded-full bg-red-600" />{match.clock ? `Live ${match.clock}` : "Live"}</span>
  </div>);
