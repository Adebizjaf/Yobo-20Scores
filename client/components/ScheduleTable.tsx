...
export default function ScheduleTable() {
  ...
  return <div className="w-full">
    ...
    <div className="grid gap-3 md:hidden">{data.map((fixture) => <div key={fixture.id} className="rounded-xl border bg-background p-3 shadow-sm"><div className="flex min-w-0 flex-wrap items-center justify-between gap-x-2 gap-y-1 text-xs text-foreground/60"><span className="font-semibold uppercase tracking-wide">{fixture.sport}</span><span className="shrink-0">{fixture.date} • {fixture.time}</span></div><div className="mt-3 grid min-w-0 grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] items-center gap-1"><div className="flex min-w-0 items-center gap-1.5"><TeamLogo name={fixture.home} logo={fixture.homeLogo} /><span className="min-w-0 truncate text-sm font-medium">{fixture.home}</span></div><span className="px-1 text-sm font-semibold text-foreground/70">vs</span><div className="flex min-w-0 items-center justify-end gap-1.5"><span className="min-w-0 truncate text-right text-sm font-medium">{fixture.away}</span><TeamLogo name={fixture.away} logo={fixture.awayLogo} /></div></div><div className="mt-2 text-xs text-foreground/60">{fixture.league}</div></div>)}</div>
